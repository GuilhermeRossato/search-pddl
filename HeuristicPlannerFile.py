...
class Heuristic_Planner(PDDL_Planner):
	...
	def solve(self, actions, initial_state, positive_goals, negative_goals):
		...
		while (len(elements) > 0):
			...
			stateId += 1
		print("Quantity inside: ", len(elements))
		return None
...
if __name__ == '__main__':
	...
	if (plan == None):
		print(time, "Expected 17, got None, False!");
	else:
		print(time, "Expected 17, got:", str(len(plan)) + ('. Correct!' if len(plan) == 17 else '. False!'))
	plan, time = planner.solve_file(dwr, pb2, True)
	if (plan == None):
